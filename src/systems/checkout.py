...
class CheckoutSystem:
    ...
    def update(self, dt: float) -> None:
        if self.checkout_state == CheckoutSystemState.WAITING_FOR_CUSTOMER and len(self.state.checkout_line) > 0:
            self.current_customer_id = self.state.checkout_line.popleft()
            customer = next(c for c in self.state.customers if c.uuid == self.current_customer_id)
